    ...
    trial(display_element, trial) {
        ...
        constructor(stim) {
          ...
          for (var i = 0; i < keys.length; i++) {
              if (typeof this[keys[i]] === "function") {
                // ã‚ªãƒ–ã‚¸ã‚§ã‚¯ãƒˆå†…ã®functionã¯ã“ã“ã§æŒ‡å®šã™ã‚‹å¿…è¦ãŒã‚ã‚‹ã€‚ãã†ã—ãªã„ã¨ã“ã“ã§å³æ™‚ã«å®Ÿè¡Œã•ã‚Œã¦ã€ãã®çµæžœãŒé–¢æ•°åã«ä»£å…¥ã•ã‚Œã‚‹
          ...
          if (this.mask === 'gauss'){
            ...
            for (let i = 0; i < gauss_width; i++) {
              for (let j = 0; j < gauss_width; j++) {
                invisible_img.data[cnt] = exp_value[i][j] * 255 // é€æ˜Žåº¦ã‚’å¤‰æ›´
                cnt = cnt + 4;
              }
            }
      ...
      class audio_stimulus extends psychophysics_stimulus{
        ...
        play(){
          ...
          if(this.context !== null){
            ...
            // ã‚ªãƒªã‚¸ãƒŠãƒ«ã®jspsychã§ã¯webaudioãŒä½¿ãˆã‚‹ã¨ãã¯æ™‚é–“ã®ãƒ‡ãƒ¼ã‚¿ã¨ã—ã¦context.currentTimeã‚’ä½¿ã£ã¦ã„ã‚‹ã€‚
            // psychophysicsãƒ—ãƒ©ã‚°ã‚¤ãƒ³ã§ã¯ã€performance.now()ã§çµ±ä¸€ã—ã¦ã„ã‚‹
      ...
      this.jsPsych.pluginAPI.setTimeout(() => {  
        if (trial.response_type === 'key'){
          ...
          console.log("Button response type")
          start_time = performance.now();
      ...
      const end_trial = () => {  
        ...
        if (typeof trial.stim_array !== 'undefined') { // The stimuli could be 'undefined' if the raf_func is specified.
          for (let i = 0; i < trial.stim_array.length; i++){
            ...
            if (stim) {
              ...
              if (typeof stim.dispose === 'function') {
                stim.dispose();
              }

              trial.stim_array[i] = null;
            }
          }
        }
        ...
        // gather the data to store for the trial //éŸ³ã®å†ç”Ÿæ™‚ã‹ã‚‰ã®åå¿œæ™‚é–“ã‚’ã¨ã‚‹ã‚ã‘ã§ã¯ãªã„ã‹ã‚‰ä¸è¦ï¼Ÿ
