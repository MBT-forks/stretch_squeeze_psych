function get_nway_afc_trials(trial_sequence, session_settings) {
    ...
    for (let i = 0; i < trial_sequence.length; i++) {
        ...
        if (mask_urls_not_loaded) {
            for (let i = 0; i < timeline_variables_cur["num_unique_masks"]; i++) {
                const paddedIndex = i.toString().padStart(3, '0');
                image_urls_to_load.push(`${timeline_variables_cur["mask_url_base"]}${paddedIndex}.png`);
            }
            mask_urls_not_loaded = false
        }
