...
def generate_trial(entry, block_config, trial_config, choice_image_urls, choice_names, class_to_url_dict, trial_types_dict=None, choice_name_to_bucket_dict=None):
  ...
  for key in block_config.keys():
    if key in trial_config or key in ["show_test_instructions", "show_warmup_complete_instructions"]:
...
def generate_trials(config_path, dirmap_path, output_js_path, write_csvs=True, condition_idx=None):
  ...
  for block_ind, block_config in enumerate(session_config):
    ...
    if "show_warmup_complete_instructions" not in block_config:
      block_config["show_warmup_complete_instructions"] = False

    splits = list(dataset_df['split'].unique())
    n_per_class_total = sum([block_config["n_trials_per_class_" + s] for s in splits])
  ...
  # assert df_no_exceptions['stimulus_filename'].is_unique, f"Duplicate stimulus filenames found among non-exception trials:\n{df_no_exceptions['stimulus_filename'].value_counts()[df_no_exceptions['stimulus_filename'].value_counts() > 1]}"
